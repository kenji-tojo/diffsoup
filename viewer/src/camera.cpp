/*
 * This code is adapted from https://github.com/sxyu/volrend/blob/master/src/camera.cpp
 */
...
void Camera::_update(bool transform_from_vecs) {
    ...
    const float CLIP_NEAR = 3.0f;

    // --- New: fix vertical FOV to 40 degrees ---
    const float fovy_deg = 40.0f;
    const float fovy_rad = glm::radians(fovy_deg);
    fy = (0.5f * height) / std::tan(fovy_rad * 0.5f);
    fx = fy; // keep square pixels; optional
    // -------------------------------------------
